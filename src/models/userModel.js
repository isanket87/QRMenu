...
async function createUser({ fullName, businessName, phoneNumber, email, password, city, state, country, role, status = true }) {
...
const findUserByEmail = async (email) => {
    const result = await pool.query('SELECT id, full_name, email, password, role, status, business_name, phone_number, city, state, country, qr_code_url FROM users WHERE email = $1', [email]);
    return result.rows[0];
};
